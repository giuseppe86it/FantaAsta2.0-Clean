/* FantaAsta2.0 — Strategy Engine A5.6.4
   Strategy Score con normalizzazione PER SLOT e dati locali facoltativi:
   qualità, titolarità LIVE, performance, profondità, costo, flessibilità,
   regolamento e scarsità reale. */
(function(){
  const ENGINE_VERSION="A5.6.4";
  ...
  function moduleSeedScore(p,module,ctx,maxFvm,maxPrice){
    const fvm=Math.max(0,Number(p?.fvm)||0),price=basePrice(p,ctx),starter=starterProb(p,ctx);
    ...
    /* FVM e prezzo sono correlati: il filtro di ingresso evita di contarli quasi
       due volte e lascia più spazio a titolarità e compatibilità col modulo. */
    const base=clamp(Math.sqrt(fvm/Math.max(1,maxFvm))*100)*.38
      +clamp(Math.sqrt(price/Math.max(1,maxPrice))*100)*.24
      +starter*.28+fit*.10;
    /* Lo storico affidabile entra già nella preselezione. In sua assenza il
       punteggio resta identico alla formula base e nessuno viene penalizzato. */
    const piRaw=ctx?.playerIntelligence?ctx.playerIntelligence(p):null;
    const history=clamp(Number(piRaw?.score)||0),reliability=clamp(Number(piRaw?.reliability)||0);
    const historyWeight=history>0?.08+.10*(reliability/100):0;
    return clamp(base*(1-historyWeight)+history*historyWeight);
  }
