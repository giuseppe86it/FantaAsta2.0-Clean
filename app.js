...
function availabilityFeedValid(feed=availabilityLiveFeed){
  return !!feed && feed.schema===AVAILABILITY_LIVE_SCHEMA && feed.complete===true && Array.isArray(feed.teams) && feed.teams.length>=15 && Array.isArray(feed.cards);
}
...
function renderFormationsView(){
  ...
  $("#formationsView").innerHTML=`
    ...
    <div class="formations-update-card ${live?"live":"base"}">
      ...
      <button type="button" class="formation-refresh-btn" aria-label="Aggiorna formazioni, infortuni e disciplina" onclick="refreshFormationsLive()" ${updating?"disabled":""}>${updating?"Aggiorno…":"Aggiorna"}</button>
    </div>
...
function liveResultHTML(p,intel=getAuctionIntel(),recommendations=null,meta=null){
  ...
  const badges=[planBadge,starterBadge].join("");
...
function selectLivePlayer(id){
  ...
  target.innerHTML=`<div class="live-player-card ${plan?.kind==="target"?"plan-recommended":""}">
    ${targetSignal}
    <div class="live-player-head"><div class="live-player-identity">${kitHTML(p.club,'live',p.club)}<div><span>${p.club} · ${p.role} · Titolarità ${Math.round(starterProbability(p).prob)}%</span><b>${esc(p.name)}${plan?.kind==="target"?' <em class="live-inline-plan">TARGET</em>':plan?.kind==="alt"?` <em class="live-inline-alt">ALT ${plan.rank}</em>`:""}</b><button type="button" class="live-watch ${isWatchlisted(p.id)?"active":""}" aria-pressed="${isWatchlisted(p.id)?"true":"false"}" aria-label="${isWatchlisted(p.id)?"Rimuovi":"Aggiungi"} ${escAttr(p.name)} ${isWatchlisted(p.id)?"dalla":"alla"} watchlist" onclick='toggleWatchlist(${idArg(p.id)})'>${isWatchlisted(p.id)?"SEGUITO":"SEGUI"}</button></div></div><strong>${riskIcon(live.risk)} ${live.risk}</strong></div>
...
function renderSquad(){
  ...
  $("#squadView").innerHTML=`
    ...
    ${fa2RecommendedLineupModuleSwitchHTML()}
    ${fa2RecommendedLineupHTML()}
...
function fa2RosterImportPreviewHTML(pending){
  ...
  return `<div class="dialog-body roster-import-dialog-body">
    <div class="safety-modal-head"><div><div class="eyebrow">FANTACALCIO CSV · A6.0.3</div><h2>Anteprima rose</h2></div><button class="ghost" type="button" aria-label="Chiudi anteprima rose" onclick="closeRosterImportDialog()">✕</button></div>
...
function fa2RosterExportPreviewHTML(pending){
  ...
  return `<div class="dialog-body roster-import-dialog-body roster-export-dialog-body">
    <div class="safety-modal-head"><div><div class="eyebrow">FANTACALCIO CSV · A5.6</div><h2>Esporta rose</h2></div><button class="ghost" type="button" aria-label="Chiudi esportazione rose" onclick="closeRosterImportDialog()">✕</button></div>
...
function renderLeagues(){
  if(!state.league){
    $("#leagueView").innerHTML=`
      ...
      <div class="league-csv-import-card primary-import">
        <div><span>FANTACALCIO CSV · A5.6</span><b>Importa automaticamente lega, squadre, rose e prezzi</b><small>Scegli il file esportato da Leghe Fantacalcio; dopo l'anteprima indicherai qual è la tua squadra.</small></div>
  ...
  $("#leagueView").innerHTML=`
    ...
    <div class="league-csv-import-card">
      <div><span>ROSE FANTACALCIO · A5.6</span><b>Importa, aggiorna o esporta le rose ufficiali</b><small>${league.teams.reduce((sum,team)=>sum+rosterForLeagueTeam(team).length,0)} assegnazioni correnti · export in sola lettura con verifica ID e prezzi</small></div>
...
function renderSettings(){
  $("#settingsView").innerHTML=`<div class="section-title"><h2>Impostazioni</h2></div>
    ...
    <div class="card clean-data-settings-card" style="margin-top:10px"><h3>Dati e copyright</h3>
      ...
      <a class="ghost clean-data-download" href="./DATI-E-COPYRIGHT.md" download>Scarica informativa tecnica</a>
    </div>
...
if("serviceWorker" in navigator) window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=2.0.0-alpha.6.0.3").catch(()=>{}));
...
function fa2StrategyContext(reg,scope="full",options={}){
  ...
  return {
    ...
    expectedPrice:(p,m)=>Math.max(configuredReservePerSlot(),Math.round(Math.min(Number(m?.price)||Number(p?.maxPrice)||neutralPrice(p),Math.max(configuredReservePerSlot(),Number(p?.fvm||0)*1.8*(.88+(Number(m?.starter)||starterProbability(p))/600))))),
...
function fa2RepairPlayerMetrics(p){
  const health=fa2AvailabilityAssessment(p),quality=clamp(100*Math.sqrt(Math.max(0,playerQuality(p))/fa2RepairPhaseMax(p)),0,100),fvmQuality=clamp(100*Math.sqrt(Math.max(0,Number(p?.fvm)||0)/fa2RepairPhaseMax(p,"fvm")),0,100),starter=fa2EffectiveStarterProbability(p,health),pi=window.FA2PlayerIntelligence?.get?.(p)||null;
  const history=Number(pi?.score)>0?clamp(Number(pi.score),0,100):(quality*.65+starter*.35),reliability=clamp(Number(pi?.reliability)||0,0,100),tier=fa2RepairTierScore(p),flexibility=clamp(Math.max(0,roleTokens(p.role).length-1)*34,0,100);
  const score=Math.round(clamp(quality*.25+fvmQuality*.12+starter*.23+history*.27+tier*.10+flexibility*.03-health.penalty,0,100));
  return {score,quality:Math.round(quality),fvmQuality:Math.round(fvmQuality),starter,history:Math.round(history),reliability:Math.round(reliability),tier,flexibility,health,pi};
}
...
function fa2RepairSwapEvaluation(player,candidate,module,reg,session,owned,metricsFor=fa2RepairPlayerMetrics){
  ...
  const severe=["long","season"].includes(current.health.severity)||player.outOfListone;
  ...
  if(current.health.severity==="short")blocks.push("infortunio breve: il giocatore va protetto");
  if(["medium","long","season"].includes(next.health.severity))blocks.push("sostituto indisponibile troppo a lungo");
  if(next.health.severity==="unknown")blocks.push("recupero del sostituto non abbastanza certo");
  const minimumGain=severe?0:7;if(improvement<minimumGain)blocks.push(`miglioramento insufficiente (${improvement>=0?"+":""}${improvement})`);
  if(!severe&&current.tier>=78&&next.tier<current.tier)blocks.push("fascia tecnica inferiore");
  if(!severe&&next.history+8<current.history)blocks.push("storico sensibilmente inferiore");
  if(!severe&&next.quality<current.quality*.82)blocks.push("valore tecnico sensibilmente inferiore");
  if(next.reliability<35)blocks.push("dati del sostituto poco affidabili");
  if(current.health.severity==="long")reasons.push(`infortunio lungo${current.health.days!=null?` · circa ${current.health.days} giorni`:""}`);
  if(current.health.severity==="season")reasons.push("rientro non previsto o stagione compromessa");
  if(player.outOfListone)reasons.push("giocatore fuori dal listone");
  if(next.starter>=current.starter+6)reasons.push(`titolarità ${current.starter}% → ${next.starter}%`);
  if(next.history>=current.history+5)reasons.push(`storico ${current.history} → ${next.history}`);
  ...
  if(next.history<current.history)warnings.push(`storico inferiore ${current.history} → ${next.history}`);
  if(next.quality<current.quality)warnings.push(`qualità inferiore ${current.quality} → ${next.quality}`);
  if(current.health.severity==="unknown")warnings.push("tempi di recupero da verificare");
...
function fa2BuildRepairAdvisor(profile,reg){
  ...
  for(const player of owned){
    ...
    rows.push({player,currentScore:evaluation.current.score,replacement:best.candidate,replacementScore:evaluation.next.score,improvement:evaluation.improvement,refund,evaluation,priority:evaluation.improvement+(player.outOfListone?35:0)+(["long","season"].includes(evaluation.current.health.severity)?20:0)});
  }
...
function fa2RepairAdvisorHTML(profile,reg,lifecycle){
  ...
  const rows=advisor.rows.length?advisor.rows.map((row,index)=>{
    ...
    const healthCurrent=current.health.injury?`${current.health.label}${current.health.days!=null?` · ~${current.health.days} giorni`:""}`:current.health.suspended?"Squalifica temporanea":"Disponibile";
    const healthNext=next.health.injury?`${next.health.label}${next.health.days!=null?` · ~${next.health.days} giorni`:""}`:next.health.suspended?"Squalifica temporanea":"Disponibile";
    ...
    return `<article class="fa2-repair-single"><header><span>SCAMBIO CONSIGLIATO ${index+1}</span><b>Miglioramento +${row.improvement}</b></header><div class="fa2-repair-swap"><span><small>VALUTA SVINCOLO</small><b>${esc(row.player.name)}</b><em>${esc(row.player.club)} · ${esc(row.player.role)} · indice ${row.currentScore}</em></span><i aria-hidden="true">→</i><span><small>OBIETTIVO</small><b>${esc(row.replacement.name)}</b><em>${esc(row.replacement.club)} · ${esc(row.replacement.role)} · indice ${row.replacementScore}</em></span></div><div class="fa2-repair-compare"><div><span>FVM</span><b>${fmt(row.player.fvm||0)} → ${fmt(row.replacement.fvm||0)}</b></div><div><span>TITOLARITÀ CORRETTA</span><b>${current.starter}% → ${next.starter}%</b></div><div><span>STORICO</span><b>${current.history} → ${next.history}</b></div><div><span>AFFIDABILITÀ DATI</span><b>${current.reliability}% → ${next.reliability}%</b></div><div class="wide"><span>DISPONIBILITÀ</span><b>${esc(healthCurrent)} → ${esc(healthNext)}</b></div></div><details class="fa2-repair-why" open><summary>Perché lo scambio è consigliato <i aria-hidden="true">⌄</i></summary><div><ul>${reasons}</ul>${warnings}<p>La mancata presenza nella probabile formazione non penalizza automaticamente infortunati brevi o squalificati.</p></div></details><div class="fa2-repair-economy"><span>Rimborso previsto <b>${row.refund==null?"—":fmt(row.refund)+" cr"}</b></span><span>MAX sostituto <b>${fmt(maxLive)} cr</b></span><span>Impegno netto <b>${net==null?"—":net>=0?fmt(net)+" cr da aggiungere":"+"+fmt(Math.abs(net))+" cr residui"}</b></span></div><div class="fa2-repair-actions"><button type="button" class="ghost" onclick='openPlayer(${idArg(row.replacement.id)})'>Apri sostituto</button>${active?`<button type="button" class="primary" onclick='fa2OpenRepairSuggestion(${idArg(row.player.id)})'>Prepara svincolo</button>`:`<button type="button" class="primary" onclick="openRepairMarket()">${session?.pausedAt?"Riprendi sessione":"Configura sessione"}</button>`}</div></article>`;
  }).join(""):`<div class="fa2-slot-note"><b>NESSUNO SCAMBIO SICURO</b> · Non emerge un miglioramento reale che superi le protezioni su fascia, storico, titolarità, salute e affidabilità dei dati.</div>`;
  return `<section class="fa2-repair-advisor"><div class="fa2-repair-advisor-head"><div><span>REPAIR STRATEGY · A5.8.2</span><b>Scambi singoli · ${esc(advisor.module?.name||"Modulo strategico")}</b><small>${advisor.availableCount} giocatori liberi confrontati · ${advisor.rows.length}/4 proposte valide · falsi miglioramenti bloccati automaticamente</small></div><button type="button" class="ghost" onclick="openRepairMarket()">${active?"Gestisci riparazione":session?.pausedAt?"Riprendi riparazione":"Configura riparazione"}</button></div>${advisor.limitReached?`<div class="fa2-slot-note"><b>LIMITE RAGGIUNTO</b> · La sessione non consente altre sostituzioni conteggiate.</div>`:rows}${fa2RepairWhatIfHTML(profile,reg)}<p class="muted">Ogni proposta è indipendente. Infortuni brevi e squalifiche non trasformano un titolare in una riserva; infortuni lunghi e stagione compromessa aumentano invece la priorità di sostituzione.</p></section>`;
}
...
function renderStrategyView(){
  ...
  if(cached&&cached.engineVersion!=="A5.6.4")cached=null;
  ...
  root.innerHTML=`<div class="fa2-hero"><span>FANTAASTA2.0 · CLEAN DATA A6.0</span><h2>Strategia</h2><p>Le analisi utilizzano esclusivamente il regolamento e i dati locali importati dall’utente. Nessuna statistica o fonte esterna viene interrogata automaticamente.</p></div>
    ${completionBanner}
    ${repairAdvisor}
    <div class="fa2-reg-strip alpha4"><div><span>Partecipanti</span><b>${sum.participants}</b></div><div><span>Bacino</span><b>${sum.analysisPool}</b></div><div><span>Budget</span><b>${sum.budget}</b></div><div><span>Rosa</span><b>${sum.roster}</b></div><div><span>Under</span><b>${sum.under}</b></div><div><span>Switch</span><b>${String(sum.switchMode).toUpperCase()}</b></div><div><span>Disponibilità</span><b>${sum.availability}</b></div><div><span>Voti</span><b>${sum.scoringSource}</b></div><div><span>Soglie gol</span><b>${sum.goalBands}</b></div><div><span>Modificatori</span><b>${sum.modifiers}</b></div></div>
